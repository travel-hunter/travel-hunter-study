...
- 본인 PC에 Linux 실습 환경을 구축한다
- 터미널 사용에 익숙해진다
- 파일·디렉토리를 자유롭게 탐색·생성·삭제·이동할 수 있다
...
세 가지 방법 중 본인 환경에 맞는 것을 선택한다.

### 1.1 Windows — WSL2 (권장)

WSL2(Windows Subsystem for Linux)는 윈도우 안에서 Ubuntu를 사용할 수 있게 해준다.
...
3. 컴퓨터 재부팅
4. Ubuntu가 자동 설치되며, 사용자명·비밀번호를 설정

> Windows 11에서는 Microsoft Store에서 "Ubuntu"를 검색해 직접 설치할 수도 있다.

### 1.2 Mac — 기본 터미널

macOS는 Unix 기반이므로 별도 설치 없이 기본 터미널을 그대로 사용하면 된다.

- `Cmd + Space` → "터미널" 검색 → 실행

### 1.3 VMware + Ubuntu Server 24.04 설치

> VMware가 이미 설치되어 있다면 1.3.3부터 진행한다.
...
> Mac 사용자는 VMware Fusion을 다운로드한다. M1/M2 Mac은 UTM 또는 Parallels를 권장한다.
...
3. ISO 파일 다운로드 (약 2GB — Desktop보다 훨씬 가볍다)

> Desktop이 아닌 **Server** 버전을 받아야 한다. 페이지를 잘 확인할 것.
...
4. Easy Install이 뜨면 무시하고 진행 (Server는 수동 설치 권장)
5. 가상머신 이름 지정 (예: `ubuntu-server-travelhunter`)
6. **디스크 크기**: 20GB (Server는 가벼워서 더 적게 잡아도 무방)
...
설치 화면이 검은색 텍스트 기반으로 나타난다. 화살표 키, Enter, Tab으로 조작한다.
...
4. **Type of install**: `Ubuntu Server (minimized)` 선택 (권장)
   - minimized는 더 가벼운 버전이다. 필요한 패키지만 직접 설치한다.
...
9. **Profile setup** (중요)
   ...
   - Username: `ubuntu` (영문 소문자 권장)
   - Password: 본인이 정한 비밀번호 (반드시 기억)
10. **Ubuntu Pro**: `Skip for now`
11. **SSH Setup**: 반드시 설치 체크
    - `Install OpenSSH server` 체크
    - 호스트(Windows)에서 SSH로 접속하기 위해 필요
...
위 메시지가 나와도 당황하지 말고 Enter를 누른다.

> **재부팅 시 ISO 제거 메시지**
> "Please remove installation media" 메시지가 뜨면 Enter를 누른다. VMware가 자동으로 제거한다.

#### 1.3.6 첫 로그인

재부팅 후 검은 화면에 다음 메시지가 나타난다.
...
- Password: 본인 비밀번호 (입력해도 화면에 표시되지 않는 것이 정상)
...
이 IP를 메모해둔다. MobaXterm 접속에 사용한다.
...
좌측에는 세션 목록, 가운데는 터미널, 좌측 패널 아래에는 SFTP 파일 탐색기가 표시된다.
...
세션을 한 번 만들어두면 좌측 사이드바에 저장된다. 다음부터는

1. 좌측 `Sessions` 패널에서 저장된 세션 더블클릭
2. 비밀번호만 입력하면 즉시 접속

> **비밀번호 저장 (선택)**
> MobaXterm이 비밀번호를 기억하게 할 수 있다. 다만 보안을 위해 마스터 패스워드 설정을 권장한다.
...
rm test.txt                  # 파일 삭제
rm -rf folder/               # 디렉토리 강제 삭제
```

> **위험 경고**
> `rm -rf /`는 시스템 전체를 삭제한다. `rm -rf`를 쓸 때는 경로를 두 번 확인해야 한다. 휴지통이 없다.
...
# 3. 디렉토리 구조 만들기 (-p: 중간 경로까지 자동 생성하는 옵션)
mkdir -p project/src/api

# mkdir(-p 없이)로도 시도해볼 것 (실패할 것)
...
nginx 웹서버를 설치해서 로그 파일 확인을 실습한다.
...
`http://localhost` 또는 서버 IP로 접속한다.

페이지를 한 번만 열어도 로그가 생성되며, 여러 번 새로고침하면 로그가 쌓인다.
...
> **실무 활용 — `tail -f`**
> 배포 후 로그를 실시간으로 보는 가장 자주 쓰는 명령어다. 에러가 발생하는 순간을 즉시 확인할 수 있다.
> 종료는 `Ctrl + C`.
...
- `journalctl`: "시스템이 켜지자마자 일어난 모든 일"을 가장 깊숙한 곳에서 꺼내온 것. systemd 기반 시스템에서 기본적으로 로그를 관리한다.
...
| `cd 경로` | 디렉토리 이동 |
| `mkdir -p 경로` | 디렉토리 생성 (중간 경로 포함) |
| `touch 파일` | 빈 파일 생성 |
| `cp -r 원본 대상` | 복사 (디렉토리는 `-r`) |
| `mv 원본 대상` | 이동·이름 변경 |
| `rm -rf 디렉토리` | 강제 삭제 (주의) |
| `cat 파일` | 전체 보기 |
| `less 파일` | 페이지 단위 보기 |
...
1. 홈 디렉토리에 `study` 디렉토리를 만들고, 그 안에 `day1`, `day2`, `day3` 하위 디렉토리를 한 번에 생성해본다.
2. `day1` 디렉토리 안에 `note.txt` 파일을 만들고, 다른 디렉토리에 복사해본다.
3. `sudo journalctl -f`로 시스템 로그를 1분간 모니터링해본 뒤, 어떤 메시지가 나오는지 확인한다.